...
import com.alibaba.datax.protobuf.BufferOuterClass;

public class ProtoBufferedRecordExchanger implements RecordSender, RecordReceiver {
...
//	private final List<Record> buffer;
	...
	private final BufferOuterClass.Buffer.Builder buffer;
	...
	public ProtoBufferedRecordExchanger(final Channel channel, final TaskPluginCollector pluginCollector) {
		assert null != channel;
...
//		this.buffer = new ArrayList<Record>(bufferSize);
		this.buffer = BufferOuterClass.Buffer.newBuilder();
		...
		try {
			ProtoBufferedRecordExchanger.RECORD_CLASS = ((Class<? extends Record>) Class
	...
	public Record createRecord() {
		try {
			return ProtoBufferedRecordExchanger.RECORD_CLASS.newInstance();
		} catch (Exception e) {
			throw DataXException.asDataXException(
					FrameworkErrorCode.CONFIG_ERROR, e);
		}
	...
	public void sendToWriter(Record record) {
		...
		this.buffer.addBuffer((BufferOuterClass.ProtobufRecord) record);
		this.bufferIndex++;
...
//		this.channel.pushAll(this.buffer);
	...
	public Record getFromReader() {
		...
		boolean isEmpty = (this.bufferIndex >= this.buffer.getBufferCount());
		if (isEmpty) {
			receive();
		}

		Record record = this.buffer.getBuffer(this.bufferIndex++);
...
//		this.channel.pullAll(this.buffer);
		this.bufferIndex = 0;
		this.bufferSize = this.buffer.getBufferCount();
	}
}
